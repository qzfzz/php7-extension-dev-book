...
注意这里的第二个参数，`sizeof("_SESSION") - 1`，一定要排除标识字符串结束的\0符。php7的第二个参数是bool型的jit标志。
...
zend_bool php_sample4_autoglobal_callback(char *name, uint name_len TSRMLS_DC)
{
    zval *sample4_val;
    int i;
    MAKE_STD_ZVAL(sample4_val);//php7中要注释掉这行
...
zend_bool php_sample4_autoglobal_callback(char *name, uint name_len TSRMLS_DC)
{
    ...
    for(i = 0; i < 10000; i++) {
    	...
    	zend_set_local_var_str("_SAMPLE4", sizeof("_SAMPLE4")-1, ztmp, 0);
