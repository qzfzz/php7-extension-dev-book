...
````c
{
    ...
    if ( /* fooval = */ zend_hash_str_find(
    		&EG(symbol_table), //这个参数是地址，如果我们操作全局作用域，则需要&EG(symbol_table)
    		"foo",
    		sizeof("foo") - 1 ))
    {
        php_printf("成功发现$foo!");
...
首先我们定义了一个指向指针的指针，然后通过zend_hash_str_find去EG(symbol_table)作用域下寻找名称为foo($foo)的变量，
如果成功找到，此函数将返回zval变量的指针。
