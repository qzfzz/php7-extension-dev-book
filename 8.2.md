...
````
如你所见，每个元素在从符号表里删除的时候(比如执行"<?php unset($foo);"操作)，都会触发ZVAL_PTR_DTOR宏代表的函数来对其进行与引用计数有关的操作。
如果元素个数不是2的整数幂形式，所以它会在函数执行时被调成成64。
...
//以下两个函数是php5中的zend_hash_update与zend_hash_add的改写
...
前两个函数与后两个函数用户添加带字符串索引的数据到HashTable中，就像我们在PHP中使用的那样:$foo['bar'] = 'baz';用C来完成便是：

````c

zend_hash_str_add(fooHashTbl, "bar", sizeof("bar"), &barZval );

````
zend_hash_str_add()和zend_hash_str_update()唯一的区别就是如果这个key已经存在了，那么zend_hash_str_add()将返回key对应的value值，而不会修改原有数据。zend_hash_add与zend_hash_update的行为与zend_hash_str_add和zend_hash_str_update的行为相似。

接下来的两个函数用于像HT中添加数字索引的数据，zend_hash_next_index_insert()函数则不需要索引值参数，而是自己直接计算出下一个数字索引值.
但是如果我们想获取下一个元素的数字索引值，也是有办法的，可以使用zend_hash_next_free_element()宏：

````c
ulong nextid = zend_hash_next_free_element(ht);
zend_hash_index_update(ht, nextid, &data );

````

### 查找
因为HashTable中有两种类型的索引值，所以需要两种类型的函数来执行find操作。
````c

ZEND_API zval* ZEND_FASTCALL zend_hash_find(const HashTable *ht, zend_string *key);
ZEND_API zval* ZEND_FASTCALL zend_hash_str_find(const HashTable *ht, const char *key, size_t len);//原PHP5中的zend_hash_find的改版
...
void hash_sample(HashTable *ht, sample_data *data1)
{
	sample_data *data2;
	ulong targetID = zend_hash_next_free_element(ht);
	if (zend_hash_index_update(ht, targetID, data1 ) == NULL ) {
			/* Should never happen */
			return;
	}
	if( !(data2 = zend_hash_index_find(ht, targetID ) )) {
		/* Very unlikely since we just added this element */
		return;
	}
	/* data1 != data2, however *data1 == *data2 */
...
ZEND_API zend_bool ZEND_FASTCALL zend_hash_str_exists(const HashTable *ht, const char *str, size_t len);//原PHP5中的zend_hash_exists
ZEND_API zend_bool ZEND_FASTCALL zend_hash_index_exists(const HashTable *ht, zend_ulong h);

````
这两个函数返回1或者0，分别代表着是否存在：
````c
if( zend_hash_exists( ht, "foo", sizeof("foo"))  )
{
	/* $foo is set */
}
else
...
````
 * \*source中的所有元素都会通过pCopyConstructor函数Copy到*target中去，我们还是以PHP语言中的数组举例，pCopyConstructor这个hook使得我们可以在copy变量的时候对他们进行一些其他操作。target中原有的与source中索引位置的数据会被替换掉，而其它的元素则会被保留，原封不动。

````c

ZEND_API void  ZEND_FASTCALL _zend_hash_merge(HashTable *target, 
	HashTable *source, 
	copy_ctor_func_t pCopyConstructor, 
	zend_bool overwrite ZEND_FILE_LINE_DC);
...
{
    //如果是字符串索引
    return ( ZSTR_VAL( hash_key ) && ZSTR_LEN( hash_key));
}
...
//生成一个名为arrht、元素为zval*类型的HashTable
zend_hash_apply(Z_ARRVAL_P(arrht), php_sample_print_zval );
...
int php_sample_print_zval_and_key(zval *val,int num_args,va_list args,zend_hash_key *hash_key)
{
	...
	{
        //如果是字符串类型的key
		PHPWRITE(ZSTR_VAL( hash_key ), ZSTR_LEN( hash_key ));
	}
	else
	{
		//如果是数字类型的key
...
ZEND_API int ZEND_FASTCALL zend_hash_get_current_key_ex(const HashTable *ht, zend_string **str_index, zend_ulong *num_index, HashPosition *pos)
        ...
        type = zend_hash_get_current_key( arrht, &key, &idx );
        if(pzval = zend_hash_get_current_data( arrht ))
        {
            /* Should never actually fail
             * since the key is known to exist. */
            continue;
        ...
        {
            /* String Key / Associative */
            PHPWRITE(ZSTR_VAL(key), ZSTR_LEN(keylen));
        } else {
            /* Numeric Key */
            php_printf("%ld", idx);
        }
...
void php_sample_print_var_hash(HashTable *arrht)
{
    ...
    zend_hash_move_forward_ex(arrht, &pos)) {
        ...
        type = zend_hash_get_current_key_ex(arrht,
                                &key,
                                &idx, &pos);
        if (!(pzval = zend_hash_get_current_data_ex(arrht &pos) )) {
            /* Should never actually fail
             * since the key is known to exist. */
            continue;
        }
...
int sample_strvec_handler(int argc, char **argv TSRMLS_DC)
{
    ...
    //初始化
    zend_hash_init(ht, argc, NULL,ZVAL_PTR_DTOR, 0);
    
    //填充数据
    while (argc) {
        zval *value;
        ZVAL_STRING( value, argv[argc] );
        argv++;
        if (zend_hash_next_index_insert(ht, value ) ) {
            /* Silently skip failed additions */
            zval_ptr_dtor(&value);
        }
    }
...
void php_sample_max(HashTable *ht )
{
    zval *pRet = zend_hash_minmax(ht, name_compare, 1);//max
   
}

````
zend_hash_compare()也许要回调函数，它的功能是将HashTable看作一个整体与另一个HashTable做比较，如果前者大于后者返回1，相等返回0，否则返回-1。
<p>int zend_hash_compare(HashTable *hta, HashTable *htb,compare_func_t compar, zend_bool ordered );
...
````
最后一个参数如果为TRUE，则会抛弃HashTable中原有的索引-键关系，将对排列好的新值赋予新的数字键值。
